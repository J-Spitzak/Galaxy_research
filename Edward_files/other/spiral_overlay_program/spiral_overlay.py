...
def SpiralPlot(galaxy_image,arm_number,pitch_angle,rotation_angle = 0.0,colorscale_option = 'n', chirality = 'CCW', spin = 5.0):
    ...
    for i in range(0,arm_number):
        ...
        #this does not work for whatever reason, these points end up resulting in a zero'd out matrix.
        #it doesn't work because all the points lie on a single line.
        '''
        original_points = np.float32([[90,290],[295,500],[500,290]])

        current_points = np.float32([[90 + (205*spin)/90,290],[295,500],[500 - (205*spin)/90,290]])

        #this works for whatever reason including updating the spin
        one = np.float32([[23,23],[29,40],[50+spin,60]])
        two = np.float32([[45,45+spin],[39,60+spin],[20,20]])

        Affine_Matrix = cv2.getAffineTransform(original_points, current_points)

        print(Affine_Matrix)
        #this is where I should multiple the array by the matrix
        x = x[:,None]
        y = y[:,None]
...
SpiralPlot(galaxy_image,arm_number,pitch_angle,rotation_angle,colorscale_option,chirality,spin_angle)
...
rot_ax = plt.axes([0.20, 0.12, 0.65, 0.03])
...
pitch_ax = plt.axes([0.20, 0.08, 0.65,0.03])
pitch_slider = Slider(pitch_ax, 'Pitch angle', 0.001, 89.125, valinit=pitch_angle)

"""
spin_ax = plt.axes([0.20, 0.04, 0.65, 0.03])
spin_slider = Slider(spin_ax, 'Spin angle', 5, 360, valinit=spin_angle)
"""
...
reset_ax = plt.axes([0.47, 0.00, 0.1, 0.04])
...
record_ax = plt.axes([0.1, 0.00, 0.1, 0.04])
...
log_ax = plt.axes([0.025, 0.60, 0.15, 0.15])
...
chir_ax = plt.axes([0.025, 0.40, 0.15, 0.15])
...
arm_ax = plt.axes([0.8255, 0.50, 0.12, 0.25])
